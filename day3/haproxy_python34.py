...
if __name__ == '__main__':
    ...
    while flag:
        ...
        select_num = input('\033[33m请选择操作条目：\033[0m')

        #######################1. 查询后端服务################################
        if select_num == '1':
            query_flag = True                                                 ###初始化查询循环标志###
            while query_flag:
                backend_index = input('\033[32;1m请输入查询的backend编号或名称(返回上层菜单,请输入b)：\033[0m')
            ...
            while backend_modify_flag:
                ...
                if backend_name:                                               ###显示对应backend下的server列表###
                    ...
                    while server_modify_flag:
                        ...
                        server_index = input('\033[32;1m请输入修改的server的编号(返回上层菜单,请输入b)：\033[0m')
            ...
            while backend_delete_flag:
                ...
                if backend_name:                                              ###显示对应backend下的server列表###
                    ...
                    while server_delete_flag:
                        ...
                        server_index = input('\033[32;1m请输入删除的server的编号(返回上层菜单,请输入b)：\033[0m')
                        if len(server_index) != 0:
                            server_index = input_handle(server_index)
                        if server_index == 'b':                               ###输入b，返还上一层###
